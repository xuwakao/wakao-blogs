...
>3. 完成``linker``的内部的初始化；
>4. 执行``linker_main``，完成一系列，包括：属性初始化，exe加载、加载依赖，符号解释，重定位等等；（详细参考：[Linker和Exe加载 : linker_main函数解释](https://github.com/xuwakao/wakao-blogs/blob/master/android-linker/android%20linker%20init(5)--exe%20load.md)）
