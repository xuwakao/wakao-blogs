...
bool soinfo::prelink_image() {
  ...
  //从.dynamic section中，获取一些基本信息（例如：soname），和计算中各种section在内存的位置
...
``prelink_image``就是从``.dynamic section``中，获取一些基本信息（例如：``soname``），和计算中各种``section``在内存的位置，为后面``relocate``做准备。